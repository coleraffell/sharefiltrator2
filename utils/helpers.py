...
def save_to_csv(
    ...
    try:
        with open(options.csv, "a", newline='', encoding="utf-8") as csvfile:
...
def get_options_from_args(args):
    ...
    return Options(
        ...
        rtfa=args.get("rtfa"),
        fedauth=args.get("fedauth"),
        query=args.get("queries"),
        refinement_filters=args.get("refinement_filters"),
        output_file=args.get("output_file"),
        save_files=args.get("save_files"),
        max_threads=args.get("max_threads"),
        max_size=args.get("max_file_size") * 1024 * 1024,
...
def setup_request_params(options: Options) -> Tuple[dict, dict]:
    ...
    cookies = {
        "rtFa": options.get("rtfa").strip('"').strip("'"),
        "FedAuth": options.get("fedauth").strip('"').strip("'"),
    }
